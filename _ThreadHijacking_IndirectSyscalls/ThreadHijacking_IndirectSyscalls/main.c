...
BOOL Inject(IN PNTAPI_FUNC pNt, IN HANDLE hProcess, IN HANDLE hThread, IN PBYTE pShellcode, IN SIZE_T sSize) {
	...
	SET_SYSCALL(pNt->NtSuspendThread); 
	...
	OKAY("[ 0x%p ] Target thread suspended.\n    [>] Syscall executed from 0x%p.", hThread, pNt->NtSuspendThread.pSyscallInstAddress);

	// https://ntdoc.m417z.com/ntgetcontextthread
	SET_SYSCALL(pNt->NtGetContextThread); 
	...
	SET_SYSCALL(pNt->NtSetContextThread); 
	STATUS = RunSyscall(hThread, &ctxThreadContext);
	if (STATUS != STATUS_SUCCESS) {
...
int main(int argc, char* argv[]) {
	...
	OKAY("[ 0x%p ] [ %d ] Found thread in process %d.\n    [>] Syscall executed from 0x%p.", hThread, dwTid, dwPid, Nt.NtOpenThread.pSyscallInstAddress);
